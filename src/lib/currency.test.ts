...
/** Normalize thousands separators (Intl output varies by host locale). */
function withoutGrouping(s: string): string {
  return s.replace(/[\s\u00A0,.]/g, "");
}

describe("formatCurrency", () => {
  it("formats whole amounts with no minor units", () => {
    // Intl inserts a locale-specific thousands separator ("," "," " " or NBSP).
    const out = formatCurrency(1234, "USD");
    expect(withoutGrouping(out)).toContain("1234");
  ...
  it("never throws on a structurally invalid code (no DB CHECK on deals.currency)", () => {
    for (const bad of ["United States", "US", "USDD", "12", "u$d"]) {
      ...
      expect(withoutGrouping(formatCurrency(1234, bad))).toContain("1234");
    }
  });
